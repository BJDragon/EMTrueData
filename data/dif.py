...
for i in range(n):
    ...
    # plt.ylabel('S11 (dB)')
    # plt.savefig(f'pngs/NO_{i + 1}.png', dpi=600)  # 保存为PNG格式，dpi设置分辨率
    # plt.show()

write_to_excel('shift_result/frequency_result_data.xlsx', frequency_result_data)
write_to_excel('shift_result/shift_result_data.xlsx', shift_result_data)
df = pd.read_excel('shift_result/shift_result_data.xlsx', header=None)
# 将DataFrame转换为JSON格式
